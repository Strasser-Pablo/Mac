...
int main()
{
    ...
    Physvector<DIM,type_data_value> pos;
    pos.Set(1,0);
    pos.Set(2,0);
    m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Set(Data_Speed_Data<DIM,type_data_value>(speed),true);
    m_data_ref.m_data.GetTopologyData().GetRefToParticleList().insert(type_particle(pos));
    ...
    typedef Policies<type_pol_layer,type_pol_part_to_key,type_pol_check_dt,type_pol_add_particle,type_pol_is_inbound_filling_layer,type_pol_is_in_domain_speed> type_pol_init;
    type_pol_init m_pol_init(m_pol_layer,m_pol_part_to_key,m_pol_check_dt,m_add_particle,m_pol_is_inbound_filling_layer,m_pol_is_in_domain_speed);

...
class Test_Data_Inteface_Domain_Rho_fluid : public CxxTest::TestSuite   //LCOV_EXCL_LINE 
{
	...
	void testInterface()
	{
		...
		v.Set(2,0);
		v.Set(3,0);
		m_inter_dom[v].GetRef().SetFluid();
		vect v2;
		v2.Set(1,1);
		v2.Set(2,0);
		v2.Set(3,0);
		m_inter_dom[v2].GetRef().SetFluid();
		TS_ASSERT(m_inter_dom[v].GetNeighbour(1,1)==&m_inter_dom[v2]);
		TS_ASSERT_DELTA(m_inter_dom.GetDomainRho(v,v2).Get(),rho_fluid.Get(),eps);
		TS_ASSERT_DELTA(m_inter_dom.GetDomainRho(v,1,1).Get(),rho_fluid.Get(),eps);
		m_inter_dom[v2].GetRef().SetAir();
		TS_ASSERT_DELTA(m_inter_dom.GetDomainRho(v,v2).Get(),rho_fluid.Get(),eps);
		TS_ASSERT_DELTA(m_inter_dom.GetDomainRho(v,1,1).Get(),rho_fluid.Get(),eps);
		m_inter_dom[v].GetRef().SetAir();
		TS_ASSERT_DELTA(m_inter_dom.GetDomainRho(v,v2).Get(),rho_air.Get(),eps);
		TS_ASSERT_DELTA(m_inter_dom.GetDomainRho(v,1,1).Get(),rho_air.Get(),eps);
	}
};

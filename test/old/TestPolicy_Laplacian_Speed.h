...
#define eps 1e-10
class TestPolicy_Laplacian_Speed : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	public:
	void test1d1()
	{
		...
		Policy_Laplacian_Speed<type_data_ref> m_pol(m_data_ref);

		v.Set(1,0);
		
		TS_ASSERT_DELTA(m_pol.Get_Laplacian_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(1),-2.0,eps);
		...
		TS_ASSERT_DELTA(m_pol.Get_Laplacian_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(1),-1.0,eps);
	}
	void test2d1()
	{
		...
		Policy_Laplacian_Speed<type_data_ref> m_pol(m_data_ref);
		TS_ASSERT_DELTA(m_pol.Get_Laplacian_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(1),-2.0,eps);
		TS_ASSERT_DELTA(m_pol.Get_Laplacian_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(2),-2.0,eps);
	}
};

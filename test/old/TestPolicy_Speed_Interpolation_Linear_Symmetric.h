...
#define eps 1e-10
class TestAlgorithms_Speed_Interpolation_Symmetric : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	public:
	void test_Layer_Initial1d1()
	{
		...
		double d=0.1238912312321;
		Physvector<1,double> pos;
		pos.Set(1,0.0-d);

		Policy_Speed_Interpolation_Linear_Symmetric<type_data_ref> m_pol(m_data_ref);
		double res1=m_pol.Get_Speed(pos).Get(1);
		pos.Set(1,0.0+d);
		double res2=m_pol.Get_Speed(pos).Get(1);
		TS_ASSERT_DELTA(res1,-res2,eps);
	}

	void test_Layer_Initial2d1()
	{
		...
		Policy_Speed_Interpolation_Linear_Symmetric<type_data_ref> m_pol(m_data_ref);
		double resx1=m_pol.Get_Speed(pos).Get(1);
		double resy1=m_pol.Get_Speed(pos).Get(2);
		pos.Set(1,0.0+d);
		pos.Set(2,0);
		double resx2=m_pol.Get_Speed(pos).Get(1);
		double resy2=m_pol.Get_Speed(pos).Get(2);
		TS_ASSERT_DELTA(resx1,-resx2,eps);
		TS_ASSERT_DELTA(resy1,resy2,eps);
		
	}

	void test_Layer_Initial2d2()
	{
		typedef Data_Base_Dim_Type<double,2> DataBase0;
		DataBase0 base0;
		typedef Data_Staggered_Left<DataBase0> DataBase;
		...
		Policy_Speed_Interpolation_Linear<type_data_ref> m_pol(m_data_ref);
		double resx1=m_pol.Get_Speed(pos).Get(1);
		double resy1=m_pol.Get_Speed(pos).Get(2);
		pos.Set(1,0.0+d);
		pos.Set(2,0);

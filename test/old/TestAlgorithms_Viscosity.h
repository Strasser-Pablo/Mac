...
class TestAlgorithms_Viscosity : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	...
	void test1d1()
	{
		...
		typedef Data_Viscosity<type_data_grid_base_spacing> type_data_visco;
		type_data_visco m_data_visco(m_data_grid_base_spacing);
		...
		m_alg.Do();
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Get(1),1-0.02,eps);
	}
	void test2d1()
	{
		const int DIM=2;
		...
		m_alg.Do();
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Get(1),1-0.02,eps);
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Get(2),1-0.02,eps);
	}
};

...
#define eps 1e-10
class TestAlgorithms_Gravity : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	public:
	~TestAlgorithms_Gravity()
	{
	...
	void test1d1()
	{
		...
		type_pol m_pol(m_data_ref,-9.81,1);
		Algorithms_Gravity<type_data_ref,type_pol> m_alg(m_data_ref,m_pol);
		m_alg.Do();
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Get(1),1-0.981,eps);
	}
	void test2d1()
	{
		const int DIM=2;
		...
		Algorithms_Gravity<type_data_ref,type_pol> alg(m_data_ref,m_pol);
		alg.Do();
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Get(2),1-0.981,eps);
	}
};

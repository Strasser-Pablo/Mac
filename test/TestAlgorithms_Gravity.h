...
class TestAlgorithms_Gravity : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	...
	void test1d1()
	{
		...
		m_alg.Do();
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Get(1),1-0.981,eps);
	}
	void test2d1()
	{
		...
		alg.Do();
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Get(2),1-0.981,eps);
	}
};

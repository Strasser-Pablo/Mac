...
#define eps 1e-10
class TestPolicy_Laplacian : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	public:
	void test1d1()
	{
		...
		vect v;
		v.Set(1,0);
		Data_Pressure_Data<double> pressure;
		pressure.Set(1.0);
		m_data_ref.m_data.GetGridData()[v].GetRef().Pressure_Set(pressure);
		...
		Policy_Laplacian<type_data_ref> m_pol(m_data_ref);

		v.Set(1,0);
		
		TS_ASSERT_DELTA(m_pol.Get_Laplacian(&m_data_ref.m_data.GetGridData()[v]).Get(),-2.0,eps);
		...
		TS_ASSERT_DELTA(m_pol.Get_Laplacian(&m_data_ref.m_data.GetGridData()[v]).Get(),-1.0,eps);
	}
	void test2d1()
	{
		...
		Policy_Laplacian<type_data_ref> m_pol(m_data_ref);
		TS_ASSERT_DELTA(m_pol.Get_Laplacian(&m_data_ref.m_data.GetGridData()[v]).Get(),-4.0,eps);
	}
};

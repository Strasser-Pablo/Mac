...
class TestAlgorithms_Convection: public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	...
	void test1d1()
	{
		...
		v.Set(1,0);
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Get(1),0.0,eps);

		speed.Set(1,-0.1);
		m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Set(Data_Speed_Data<1,double>(speed));
		m_data_ref.m_data.GetGridData()[v].GetRef().SetFluid();
		
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Get(1),-0.1,eps);
	}
	void test2d1()
	{
		...
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Get(1),-0.875,eps);
		TS_ASSERT_DELTA(m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Get(2),-0.875,eps);
	}
};

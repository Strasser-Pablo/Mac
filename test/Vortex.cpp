...
class Algorithm_Extrapolate_Boundary : public Policy
{
   ...
   const type_speed_value& m_viscosity;
   public:
   Algorithm_Extrapolate_Boundary(Data data, const Policy& pol): Policy(pol),m_grid(data.m_data.GetGridData()),m_1_h(data.m_data.GetGridData().m_h.GetRef_Inv()),m_h(data.m_data.GetGridData().m_h.GetRef()),m_t(data.m_data.GetTimingData().m_t),m_viscosity(data.m_data.GetGridData().m_viscosity)
   {
   }
...
//	const int NBSpeed=2;
   ...
   m_data_viscosity.m_viscosity=1;
   ...
   for(int i=-imax;i<=imax+1;++i)
   {
       for(int j=-jmax;j<=jmax+1;++j)
       {
           ...
           {
                type_data_value x=i*spacing;
                type_data_value y=j*spacing;
                type_data_value val;
                if(dim==1)
                {
                    x-=0.5*spacing;
                ...
                type_data_value r=sqrt(x*x+y*y);
                if(dim==1)
                {
                    val=-y/r;
                }
           ...
           if(j>=-jmax&&j<=jmax&&i>=-imax&&i<=imax)
           {
               type_data_value x=i*spacing;
               type_data_value y=j*spacing;
               type_data_value r=sqrt(x*x+y*y);
...
//	typedef Algorithms_RungeKutta_RK2_TVD<type_data_ref,type_pol_ODE> type_alg_ODE;
   ...
   typedef Algorithms<type_alg_solve_grid,type_alg_solve_force_pressure>  type_alg_with_force;
   type_alg_with_force m_alg_with_force(m_alg_solve_grid,m_alg_solve_force_pressure);

   //Policy ODE integrator
      typedef Policies<type_alg_with_force,type_sol_after_extrapolate> type_pol_ODE;
      type_pol_ODE m_pol_ODE(m_alg_with_force,m_alg_after_extrapolate);

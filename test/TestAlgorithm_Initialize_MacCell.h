...
class TestAlgorithmes_Initialize_MacCell : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	...
	void test_Layer_Initial1()
	{
		...
		typedef Datas<DataBase,type_cell_type,type_grid_speed,type_grid_pressure,type_empt_lay> type_datas;
		type_datas m_datas(base,D,m_grid_speed,m_grid_pressure,m_empt_lay);
		...
		TS_ASSERT(m_data_ref.m_data.GetGridData()[v].GetRef().GetIsLayerEmpty());
	}
};

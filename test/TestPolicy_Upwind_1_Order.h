...
#define eps 1e-10
class TestPolicy_Upwind_1_Order : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	public:
	void test1d1()
	{
		...
		type_pol_speed m_pol(m_data_ref);
		Policy_Upwind_1_Order<type_data_ref,type_pol_speed> m_pol_conv(m_data_ref,m_pol);
		TS_ASSERT_DELTA(m_pol_conv.Get_Convection_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(1),-1.0,eps);

		v.Set(1,0);
		speed.Set(1,-0.1);
		m_data_ref.m_data.GetGridData()[v].GetRef().Speed_Set(Data_Speed_Data<1,double>(speed));
		TS_ASSERT_DELTA(m_pol_conv.Get_Convection_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(1),0.11,eps);
	}
	void test2d1()
	{
		...
		type_pol_speed m_pol(m_data_ref);
		Policy_Upwind_1_Order<type_data_ref,type_pol_speed> m_pol_conv(m_data_ref,m_pol);
		TS_ASSERT_DELTA(m_pol_conv.Get_Convection_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(1),-1.25,eps);
		TS_ASSERT_DELTA(m_pol_conv.Get_Convection_Speed(&m_data_ref.m_data.GetGridData()[v]).Get(2),-1.25,eps);
	}
};

...
class TestAlgorithms_Create_Fluid_Particle : public CxxTest::TestSuite  //LCOV_EXCL_LINE 
{
	...
	void test_Layer_Initial1()
	{
		...
		speed.Set(2,5.0);
		speed.Set(3,6.0);
		m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Set(Data_Speed_Data<3,double>(speed));
		...
		v.Set(1,2);
		Data_Speed_Data<3,double> speed_ret=m_data_ref.m_data.GetGridData()[v].Speed_GetRef().Speed_Get();

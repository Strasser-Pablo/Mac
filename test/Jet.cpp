...
public:
    Algorithm_Init_Perturbation(Data data, const Policy& pol,type_speed_value speedy,type_speed_value epsit,type_speed_value epsmax,int xmax): Policy(pol),m_grid(data.m_data.GetGridData()),m_1_h(data.m_data.GetGridData().m_h.GetRef_Inv()),m_h(data.m_data.GetGridData().m_h.GetRef()),m_t(data.m_data.GetTimingData().m_t),m_dt(data.m_data.GetTimingData().m_dt)
      ,m_speedy(speedy),m_epsit(epsit),m_epsmax(epsmax),m_x(xmax)
    {
        srand(1);
    }
    void Do()
    {
        ...
        for(int i=0;i<m_x;++i)
        {
            ...
            }while(abs(v10+eps)>m_epsmax);
            type_speed_value v1=v10+eps;

            do{
                eps=m_epsit*m_dt*Rand();
            }while(abs(v20+eps-m_speedy)>m_epsmax);
...
template <typename Data,typename Policy>
class Algorithm_DelParticle : public Policy
{
...
public:
        Algorithm_DelParticle(Data data, const Policy& pol,double ymin): Policy(pol),m_list(data.m_data.GetTopologyData().GetRefToParticleList()),m_y(ymin)
    {
    }
    void Do()
    {
        ...
        while(it!=m_list.end())
        {
            if((*it).GetParticlePos().Get(2)<m_y)
            {
                it=m_list.erase(it);
            }
            else
...
int main()
{
    ...
    typedef Policies<type_pol_layer,type_pol_part_to_key,type_pol_check_dt,type_pol_add_particle,type_pol_is_inbound_filling_layer,type_pol_is_in_domain_speed,type_pol_depth> type_pol_init;
    type_pol_init m_pol_init(m_pol_layer,m_pol_part_to_key,m_pol_check_dt,m_add_particle,m_pol_is_inbound_filling_layer,m_pol_is_in_domain_speed,m_pol_depth);
    ...
    typedef Algorithm_DelParticle<type_data_ref,type_pol_init> type_alg_del_particle;
    type_alg_del_particle m_alg_del_particle(m_data_ref,m_pol_init,0);
    ...
    type_alg_perturb m_alg_perturb(m_data_ref,m_pol_init,val_speed,1.0,0.01,xmax);
...
#else
    type_alg_output m_alg_output(m_data_ref,m_pol_output,iload,true);
#endif

...
class Algorithms_Layer_Initial_With_Particle : public Policy
{
	...
	public:
	Algorithms_Layer_Initial_With_Particle(DataType data) :m_data(data.m_data), m_grid(m_data.GetGridData()), m_list(m_data.GetTopologyData().GetRefToParticleList())
	{
	}
	void Do()
	{
		...
		{
			m_grid[m_data.GetKeyFromParticle(*it)].GetRef().SetLayer(GetLayerParticleInitial());
		}
	}
};

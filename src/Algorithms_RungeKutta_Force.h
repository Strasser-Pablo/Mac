...
#ifndef Algorithms_RungeKutta_Force_H
#define Algorithms_RungeKutta_Force_H
//Need 3 speed and 1 acceleration
template <typename Data,typename Policy>
class Algorithms_RungeKutta_Force : public Policy
{
    ...
    public:
    Algorithms_RungeKutta_Force(Data data,Policy& pol): Policy(pol),m_grid(data.m_data.GetGridData()),m_dt(data.m_data.GetTimingData().m_dt),m_t(data.m_data.GetTimingData().m_t)
    {
    }
    Algorithms_RungeKutta_Force(Data data, Policy&& pol): Policy(pol),m_grid(data.m_data.GetGridData()),m_dt(data.m_data.GetTimingData().m_dt),m_t(data.m_data.GetTimingData().m_t)
    {

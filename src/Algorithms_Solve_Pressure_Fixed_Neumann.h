#ifndef Algorithms_Solve_Pressure_Fixed_Neumann_H
#define Algorithms_Solve_Pressure_Fixed_Neumann_H
#include <cmath>
#include <map>
#include <deque>
#include <sys/times.h>
...
class Algorithms_Solve_Pressure_Fixed_Neumann: public Policy
{
    ...
    bool b=false;
    public:
    Algorithms_Solve_Pressure_Fixed_Neumann(DataType data,const Policy& pol) : Policy(pol),m_grid(data.m_data.GetGridData()),m_1_h(data.m_data.GetGridData().m_h.GetRef_Inv()),m_h(data.m_data.GetGridData().m_h.GetRef())
    {
    }
    ~Algorithms_Solve_Pressure_Fixed_Neumann()
    {
        delete[] m_offset;
        delete[] m_indice;
        delete[] m_value;
            ...
            if(it.data().CellType_GetRef().GetIsInDomain())
            {
                ...
                for(int i=1;i<=type_dim;++i)
                {
                    for(int s=-1;s<=1;s+=2)
                    {
                        ...
                        {
                            it.data().GetNeighbour(i,s).Pressure_GetRef().Pressure_Set(type_pressure(res[lay_cur]));
                        }
                    }
                }
            }
